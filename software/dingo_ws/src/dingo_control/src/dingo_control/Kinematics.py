...
def leg_explicit_inverse_kinematics(r_body_foot, leg_index, config):
    ...
    _sin_a2 = sin(config.phi)*config.L1/len_A if len_A > 1e-9 else 1.0
    if _sin_a2 > 1.0 or _sin_a2 < -1.0:
        rospy.logwarn_throttle(1, 'target coordinate: [%f %f %f] too close to the '
                                  'hip axis (len_A %.4f); clamping', x, y, z, len_A)
        _sin_a2 = max(-1.0, min(1.0, _sin_a2))
    a_2 = asin(_sin_a2)
    ...
    if len_B >= (config.L2 + config.L3):
        len_B = (config.L2 + config.L3) * 0.99
        rospy.logwarn_throttle(1, 'target coordinate: [%f %f %f] too far away', x, y, z)
